...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        if msg.topic == "sensor/fall_raw":
            ...
            try:
                # BUG FIX: 'return' sớm trước đây nằm trước finally
                # → _waiting_for_fall_raw không được reset.
                # Giờ toàn bộ logic nằm trong try, finally luôn chạy.
                decoded = _decode_fall_raw_binary(msg.payload)
                if not decoded:
                    print("❌ Không thể decode fall_raw binary")
                    # KHÔNG return ở đây — để finally reset flag
                else:
                    ...
                    result = _process_fall_raw_with_model(decoded)
                ...
                # Luôn reset dù decode fail, exception, hay bất kỳ path nào
                with _fall_raw_state_lock:
                    _waiting_for_fall_raw = False
