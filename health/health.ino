...
#define PRE_SAMPLES         160
#define POST_SAMPLES        160
...
#define BINARY_BUF_SIZE 6000

#define PPG_SENSOR_SAMPLE_RATE_HZ 50
#define PPG_FIFO_SAMPLE_AVERAGE   2
...
#define SPO2_MIN_IR_P2P          700
#define SPO2_MIN_RED_P2P         300

#define SPO2_MIN_ACDC              0.0035f
...
void readPpgStream() {
  ...
  while (max30102.available()) {
    ...
    unsigned long sampleIntervalMs = (unsigned long)(1000.0f / ppgMeasuredFs + 0.5f);
    if (sampleIntervalMs < 1) sampleIntervalMs = 1;

    unsigned long ts = (ppgDataLastTs == 0) ? now : (ppgDataLastTs + sampleIntervalMs);
...
bool checkPPGQuality(PPGQuality& q) {
  ...
  float timingFs = (ppgMeasuredFs > 30.0f && ppgMeasuredFs < 80.0f) ? ppgMeasuredFs : (float)PPG_SENSOR_SAMPLE_RATE_HZ;
  unsigned long expectedDuration = (unsigned long)((SPO2_WINDOW_SIZE - 1) * 1000.0f / timingFs);
  unsigned long actualDuration   = lastTs - firstTs;
  bool severeTiming = actualDuration > expectedDuration * 1.60f ||
                      actualDuration < expectedDuration * 0.50f;
  bool irregularTiming = actualDuration > expectedDuration * 1.15f ||
                         actualDuration < expectedDuration * 0.85f;
  ...
  q.maxJerk      = maxJerk;
  q.badMotionRate = motionSampleCount > 0 ? (float)badMotionCount / motionSampleCount : 0.0f;
  q.measuredFs   = ppgMeasuredFs;

  if (severeTiming)                         { strcpy(q.reason, "IRREGULAR_TIMING");      return false; }
  ...
  strcpy(q.reason, irregularTiming ? "GOOD_TIMING_WARN" : "GOOD");
  q.ok = true;
  return true;
}
...
void sendSensorWindow() {
  ...
  if (!goodSignal) {
    Serial.printf("[SPO2] Quality xau, khong gui: %s | fs=%.1f ir_mean=%.0f red_mean=%.0f ir_p2p=%.0f red_p2p=%.0f acdc=%.4f/%.4f motion=%.3f jerk=%.2f\n",
                  q.reason, q.measuredFs, q.irMean, q.redMean,
                  q.irP2P, q.redP2P, q.irACDC, q.redACDC,
                  q.badMotionRate, q.maxJerk);
    ppgNewSamples = 0;
    return;
  }
...
void setup() {
  ...
  if (!max30102.begin(I2C_MAX, I2C_SPEED_FAST)) {
    ...
    max30102.setup(0x60, PPG_FIFO_SAMPLE_AVERAGE, 2, PPG_SENSOR_SAMPLE_RATE_HZ, 411, 8192);
    max30102.setPulseAmplitudeRed(0x60);
    max30102.setPulseAmplitudeIR(0x60);
